...
class FinnDataset(CustomDataset):
    global CLASSES
    CLASSES={'Birch', 'Ground', 'Lake', 'Obstacle', 'Pine', 'Spruce', 'Track', 'Tree', 'background'}
    ...
    def find_closest_color(self,color, color_mapping):
        min_distance = float('inf')
        closest_color = None
        for mapped_color in color_mapping:
            distance = self.euclidean_distance(color, mapped_color)
            if distance < min_distance:
                min_distance = distance
                closest_color = mapped_color
    ...
    def _det2json(self, results):
        ...
        for idx in range(len(self)):
            ...
            new_filename = filename.replace("val", "finnwood_panoptic_val")
