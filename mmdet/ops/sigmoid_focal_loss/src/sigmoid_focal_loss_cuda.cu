...
at::Tensor SigmoidFocalLoss_forward_cuda(const at::Tensor &logits,
  ...
  // dim3 grid(std::min<int>(at::cuda::ATenCeilDiv(losses_size, 512), 4096));
  // dim3 grid(
  //     std::min(THCCeilDiv((int64_t)losses_size, (int64_t)512), (int64_t)4096));
  dim3 grid(
    std::min<int>((losses_size + 512 - 1) / 512, 4096));
  dim3 block(512);

  if (losses.numel() == 0) {
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
      printf("CUDA error in SigmoidFocalLoss_forward_cuda: %s\n", cudaGetErrorString(error));
      exit(-1);
    }
    return losses;
  }
  ...
  cudaError_t error = cudaGetLastError();
  if (error != cudaSuccess) {
    printf("CUDA error in SigmoidFocalLoss_forward_cuda kernel execution: %s\n", cudaGetErrorString(error));
    exit(-1);
  }
  return losses;
}
...
// dim3 grid(std::min<int>(at::cuda::ATenCeilDiv(d_logits_size, 512), 4096));
  // dim3 grid(std::min(THCCeilDiv((int64_t)d_logits_size, (int64_t)512),
  //                    (int64_t)4096));
  ...
  if (d_logits.numel() == 0) {
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
      printf("CUDA error in SigmoidFocalLoss_backward_cuda: %s\n", cudaGetErrorString(error));
      exit(-1);
    }
    return d_logits;
  }
  ...
  cudaError_t error = cudaGetLastError();
  if (error != cudaSuccess) {
    printf("CUDA error in SigmoidFocalLoss_backward_cuda kernel execution: %s\n", cudaGetErrorString(error));
    exit(-1);
  }
  return d_logits;
}
