...
from labelsFinWood import labels as cs_labels
from pycococreatortools import pycococreatortools as pct

parser = argparse.ArgumentParser(description="Convert FinnWood to coco format")
parser.add_argument("root_dir", metavar="ROOT_DIR", type=str, help="Root directory of Freiburg")
...
_INSTANCE_EXT = ".png"
_IMAGE_EXT = ".jpg"
_EVAL = "finnwood_panoptic_val"

def main(args):
    print("Loading Finnwood from", args.root_dir)
    global _EVAL
    ...
    for split, (split_img_subdir, split_msk_subdir) in _SPLITS.items():
        ...
        # print(img_base_dir,msk_base_dir)
...
def _get_images(base_dir):
    img_list = []
    # print(path.join(base_dir, "*" + _INSTANCE_EXT))
...
class _Worker:
    ...
    def __call__(self, img_desc):
        ...
        with Image.open(path.join(self.msk_base_dir, img_id + _INSTANCE_EXT)) as lbl_img:
            ...
            # lbl = cv2.resize(np.array(lbl_img), (1280, 640), interpolation=cv2.INTER_NEAREST)
        ...
        for color_id in color_ids:
            ...
            #     iscrowd_i = False
            iscrowd_i=False
            for x in range(len(cs_labels)):
                p=cs_labels[x].color
                if(p[0]==color_id[0] and p[1]==color_id[1] and p[2]==color_id[2]):
                    cls_i=x
                    iscrowd_i=cs_labels[cls_i].hasInstances
                    break
        ...
        if 'validation' == self.msk_base_dir.split('/')[-2]:
            # print("jjjdjd")
            Image.fromarray(lbl).save(path.join(_EVAL, img_id + '.jpg'))
            print(path.join(self.img_base_dir, img_id + _IMAGE_EXT))
            with Image.open(path.join(self.img_base_dir, img_id + _IMAGE_EXT)) as lbl_img:
                # img = cv2.resize(np.array(lbl_img), (1280, 384))
                img=lbl_img
                # error
                # print(path.join(self.img_dir, img_id + ".jpg"))
                # Image.fromarray(img).save(path.join(self.img_dir, img_id + ".jpg"))
                img.save(path.join(self.img_dir, img_id + ".jpg"))


        else: 
            shutil.copy(path.join(self.img_base_dir, img_id + _IMAGE_EXT),
                        path.join(self.img_dir, img_id + ".jpg"))
